...
export function demoBlueprint(input: ExperienceInput): ExperienceBlueprint {
  const aboutLower = input.about.toLowerCase();
  const groundedTerm = (aboutLower.match(/[a-z][a-z'-]{4,}/g) || []).find(term => !["about", "every", "their", "there", "which", "where", "would", "could", "always", "makes", "person", "quiet", "small"].includes(term)) || "personality";
  const anchor = aboutLower.includes("lahore") ? "Lahore inside joke" : aboutLower.includes("cricket") ? "competitive cricket energy" : `${groundedTerm} memory`;
  ...
  return {
    ...
    scenes: [
      ...
      { type: "hobby", render_mode: input.relationship === "Partner" ? "polaroid" : "dashboard", importance: "supporting", visual_concept: input.about.toLowerCase().includes("cricket") ? "cricket_scoreboard" : `${groundedTerm}_field_notes`, asset_direction: input.about.toLowerCase().includes("cricket") ? "Night-stadium green, chalk lines, scoreboard numerals, crisp monospace labels." : `A tactile visual study of the ${groundedTerm} detail, warm editorial texture, no generic sports imagery.`, asset_tokens: assetTokensFor(input.relationship === "Partner" ? "polaroid" : "dashboard"), interaction: "tap_to_advance", setup: `A completely objective report on the ${groundedTerm} detail.`, beats: ["AGE: +1", `${groundedTerm.toUpperCase()}: MEMORABLE`, "MATURITY: QUESTIONABLE"], punchline: `The ${groundedTerm} evidence is overwhelming.`, pacing: "quick", confirmed_details: input.about.toLowerCase().includes("cricket") ? ["cricket"] : [groundedTerm] },
      { type: "inside_joke", render_mode: input.about.toLowerCase().includes("lahore") ? "map" : "journal", importance: "climax", visual_concept: input.about.toLowerCase().includes("lahore") ? "gps_recalculation" : `${groundedTerm}_postscript`, asset_direction: input.about.toLowerCase().includes("lahore") ? "Indigo street-map grid, copper route line, two pins, slightly misregistered print texture." : `Ruled notebook paper with a handwritten ${groundedTerm} reference, torn corner, intimate feeling.`, asset_tokens: assetTokensFor(input.about.toLowerCase().includes("lahore") ? "map" : "journal"), interaction: "tap_to_reveal", setup: input.about.toLowerCase().includes("lahore") ? "GPS INITIALIZING... DESTINATION: LAHORE" : `The file marked: only we would understand the ${groundedTerm} reference.`, beats: input.about.toLowerCase().includes("lahore") ? ["RECALCULATING...", "RECALCULATING...", "3 HOURS LATER", "CURRENT LOCATION: UNKNOWN"] : [`A ${groundedTerm} memory too specific to explain.`, "A punchline that belongs to both of you."], punchline: input.about.toLowerCase().includes("lahore") ? "One city. Two idiots. Zero sense of direction." : `No context. Somehow, perfect: ${groundedTerm} edition.`, pacing: "slow_build", confirmed_details: input.about.toLowerCase().includes("lahore") ? ["Lahore", "three hours"] : [groundedTerm] },
