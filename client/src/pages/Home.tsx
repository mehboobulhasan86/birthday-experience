...
const blankBlueprint = (): Blueprint => ({
  ...demoBlueprint(BLANK_INPUT),
  recipient: { name: "", nickname: "", relationship: "Other" },
  about: "",
  message: "",
  tone: "everything",
});
...
export default function Home() {
  ...
  const [form, setForm] = useState<Blueprint>(blankBlueprint);
  ...
  const beginCreator = () => { setForm(blankBlueprint()); setPublishedSlug(null); setPhotoUpload(null); setMode("creator"); setStep(1); window.scrollTo({ top: 0, behavior: "smooth" }); };
  ...
  return (
    <div className="app-shell">
      ...
      {mode === "share" && <SharePage blueprint={form} slug={publishedSlug ? `/birthday/${publishedSlug}` : slug} onPreview={() => { setMode("recipient"); setScene(0); }} onCreate={() => { setForm(blankBlueprint()); setPublishedSlug(null); setPhotoUpload(null); setMode("creator"); setStep(1); }} />}
