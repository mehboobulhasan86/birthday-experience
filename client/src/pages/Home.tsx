...
function Recipient({ blueprint, scene, setScene, mapRevealed, setMapRevealed, sound, setSound, onClose, onShare }: { blueprint: Blueprint; scene: number; setScene: (scene: number) => void; mapRevealed: boolean; setMapRevealed: (value: boolean) => void; sound: boolean; setSound: (value: boolean) => void; onClose: () => void; onShare: () => void }) { const name = blueprint.recipient.nickname || blueprint.recipient.name; const next = () => setScene(Math.min(scene + 1, 5)); const previous = () => setScene(Math.max(scene - 1, 0)); return <main className={`recipient-world scene-${scene}`}><button className="recipient-close" onClick={onClose}><X size={17} /></button><button className="sound-toggle" onClick={() => setSound(!sound)}>{sound ? <Volume2 size={15} /> : <Volume2 size={15} />} {sound ? "Sound on" : "Sound off"}</button><div className="recipient-scene"><div className="scene-index">{String(scene + 1).padStart(2, "0")} <span>/ 06</span></div>{scene === 0 && <OpenScene onOpen={next} name={name} />}{scene === 1 && <NicknameScene name={name} onNext={next} />}{scene === 2 && <CricketScene onNext={next} />}{scene === 3 && <MapScene revealed={mapRevealed} setRevealed={setMapRevealed} onNext={next} />}{scene === 4 && <MessageScene message={blueprint.message} onNext={next} />}{scene === 5 && <CelebrationScene name={name} message={blueprint.message} onShare={onShare} />}</div><div className="recipient-controls">{scene > 0 && <button onClick={previous}><ArrowLeft size={16} /> Previous</button>}<div className="scene-dots">{[0,1,2,3,4,5].map((dot) => <span key={dot} className={dot === scene ? "active" : ""} />)}</div>{scene < 5 && <button onClick={next}>Continue <ArrowRight size={16} /></button>}</div></main>; }
...
function CricketScene({ onNext }: { onNext: () => void }) { return <div className="cricket-scene"><div className="stadium-glow" /><div className="recipient-kicker">SCENE 02 / PERFORMANCE REVIEW</div><h2>Another year.<br /><em>Still undefeated<br />in your own head.</em></h2><div className="scoreboard"><div className="score-head"><span>SHANI XI</span><span>LIVE / 01</span></div><div className="score-row"><span>AGE</span><b>+1</b></div><div className="score-row"><span>WICKETS</span><b>0</b></div><div className="score-row"><span>CONFIDENCE</span><b className="copper">MAX</b></div><div className="score-row"><span>MATURITY</span><b className="error">ERROR</b></div></div><button className="scene-cta" onClick={onNext}>There’s more <ArrowRight size={16} /></button></div>; }
function MapScene({ revealed, setRevealed, onNext }: { revealed: boolean; setRevealed: (value: boolean) => void; onNext: () => void }) { return <div className="map-scene"><div className="recipient-kicker">SCENE 03 / THE INCIDENT</div><h2>One city.<br /><em>Zero sense of direction.</em></h2><div className={`map-canvas ${revealed ? "revealed" : ""}`}><div className="map-grid" /><div className="map-label label-lahore">LAHORE</div><div className="map-label label-current">CURRENT LOCATION: UNKNOWN</div><div className="route-line" /><MapPin className="map-pin pin-start" size={19} /><MapPin className="map-pin pin-end" size={19} /></div>{!revealed ? <button className="map-reveal" onClick={() => setRevealed(true)}><MapPin size={15} /> Recalculate route</button> : <><div className="map-punchline">3 HOURS LATER<span>...</span></div><button className="scene-cta" onClick={onNext}>But jokes aside <ArrowRight size={16} /></button></>}</div>; }
